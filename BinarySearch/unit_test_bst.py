...
class UnitTestTemplate(unittest.TestCase):
    ...
    def test_remove(self):
        bst = create_bst_from_list(arrList1)  # [5, 18, 1, 8, 14, 16, 13, 3]

        bst.remove(5)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 5: " + self.print_tree(
            bst._root))
        bst.remove(14)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 14: " + self.print_tree(
            bst._root))
        bst.remove(8)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 8: " + self.print_tree(
            bst._root))
        bst.remove(16)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 16: " + self.print_tree(
            bst._root))
        bst.remove(13)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 13: " + self.print_tree(
            bst._root))
        bst.remove(1)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 1: " + self.print_tree(
            bst._root))
        bst.remove(18)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 18: " + self.print_tree(
            bst._root))
        bst.remove(3)
        self.assertTrue(bst.is_valid, "Incorrect tree after removing key = 3: " + self.print_tree(
            bst._root))

        self.assertIsNone(bst._root, "Incorrect tree after removing all values: " + self.print_tree(
            bst._root))
